...
backup_file = os.path.join(BASE_DIR, "studio-web", "data", "course-data.backup.js")
target_file = os.path.join(BASE_DIR, "studio-web", "data", "course-data.js")

with open(backup_file, "r", encoding="utf-8") as f:
    backup_code = f.read()

start_idx = backup_code.find("[")
end_idx = backup_code.rfind("]") + 1
communities = json.loads(backup_code[start_idx:end_idx])
...
ultimate_comm = next((c for c in communities if c["id"] == "ultimateeditors"), None)
if not ultimate_comm:
    ultimate_comm = {
        "id": "ultimateeditors",
    ...
    communities.append(ultimate_comm)

existing_devin_idx = next((i for i, c in enumerate(ultimate_comm["courses"]) if c["id"] == "devin-jatho-editing-masterclass"), -1)
if existing_devin_idx != -1:
    ultimate_comm["courses"][existing_devin_idx] = devin_course
else:
    ultimate_comm["courses"].append(devin_course)

output_code = "window.COMMUNITIES_DATA = " + json.dumps(communities, indent=2, ensure_ascii=False) + ";\n\nwindow.COURSE_DATA = window.COMMUNITIES_DATA[0].courses[0];\n"
with open(target_file, "w", encoding="utf-8") as f:
    f.write(output_code)

print("✅ Successfully generated course-data.js with 2 communities and 4 courses (123 lessons total)!")
