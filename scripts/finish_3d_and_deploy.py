...
def resolve_loom_stream_url(loom_share_url):
    ...
    try:
        p_req = urllib.request.Request(f"https://www.loom.com/share/{lid}", headers={"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)"})
...
for mod in target_course.get('modules', []):
    ...
    for les in mod.get('lessons', []):
        ...
        # If file already downloaded on disk and > 10MB, check if already in drive
        needs_download = not (os.path.exists(file_path) and os.path.getsize(file_path) > 1000000)
        
        if needs_download:
            print(f"\n📥 Downloading missing: '{les_title}' from {video_url[:60]}...")
            success = False
            if "loom.com" in video_url:
                ...
                cmd = ["yt-dlp", "-o", file_path, "--force-overwrites", "--extractor-args", "youtube:player_client=android,web", "-f", "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best", video_url]
                p = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
                if os.path.exists(file_path) and os.path.getsize(file_path) > 10000:
                    success = True
        ...
        if os.path.exists(file_path) and os.path.getsize(file_path) > 10000:
            ...
            # Check if this lesson has its gdriveId
            if not les.get('gdriveId') or not les.get('inDrive'):
                print(f"     ☁️ Uploading {safe_name} ({file_mb:.2f} MB) to Google Drive...")
                ...
                if gid:
                    ...
                    les['inDrive'] = True
                    updated_count += 1
                    print(f"     🎯 Updated Google Drive ID: {actual_id}")
...
print(f"\n🎉 SUCCESS: All lessons mapped! Total new uploads in this pass: {updated_count}")
